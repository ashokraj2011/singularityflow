...
1. Run `singularity-flow nextsteps --json` to show state and prerequisites. If it names `singularity-flow wm ensure`, explain that semantic generation may start a repository-reading, file-writing Copilot agent and ask for explicit consent. Do not start it while waiting. After consent, run the exact `wm ensure` command and then `singularity-flow next --task "<current objective>"`; after a decline, stop with no state change. Otherwise run `singularity-flow next --task "<current objective>"`.
