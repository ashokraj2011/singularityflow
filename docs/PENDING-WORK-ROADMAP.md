...
| Fast onboarding and safe Git | M0–M5 code-local contracts now exist, including exact authority attachment/recovery, typed and cached Git reads, no-discovery/AST/model Story start, safe optional UX contracts, deterministic hashed 1k/10k/100k fixtures, exact-commit evidence registration, and an executable witness inventory (49/50 represented); the first immutable Darwin/arm64 controlled report passed every registered local budget; M5 rejects unverified/copyable adapter flags in favor of exact-runtime external attestations; and `workspace current --git-shadow` now provides the first content-free legacy/typed migration comparison while leaving the reference path authoritative and default | Complete AC-035 with controlled network, office, VS Code-host, Linux, and Windows evidence; certify the live organizational bootstrap provider and the four real M5 adapters; collect cross-platform workspace shadow evidence and migrate additional high-count readers one bounded projection at a time; then perform the independent enablement review | [Operator guide](FAST-ONBOARDING-AND-GIT-PERFORMANCE.md) and [implementation plan](FOS-IMPLEMENTATION-PLAN.md) |
