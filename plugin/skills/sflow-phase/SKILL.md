...
8. Run `singularity-flow phase publish <phase>`. Commit composition, prompt snapshot, and sanitized `telemetry/<phase>-gen<N>.json` without raw traces or conversation identifiers. Current-response usage may be `pending` until submit reconciles it; use `--usage-json` only for exact external records.
