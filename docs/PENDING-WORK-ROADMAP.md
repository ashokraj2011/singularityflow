...
| Fast onboarding and safe Git | M0–M5 code-local contracts now exist, including exact authority attachment/recovery, typed and cached Git reads, no-discovery/AST/model Story start, safe optional UX contracts, deterministic hashed 1k/10k/100k fixtures, exact-commit evidence registration, and an executable witness inventory (49/50 represented); the first immutable Darwin/arm64 controlled report passed every registered local budget; M5 rejects unverified/copyable adapter flags in favor of exact-runtime external attestations; all four planned high-count read surfaces plus Story-start preflight now have content-free legacy/typed shadow comparison; editor snapshots reuse their authoritative revision instead of repeating branch, HEAD, and changed-path Git reads; and Workspaces now provides one repository-bound onboarding/Git surface with inline outcomes and recovery help. The reviewed extension plan records the remaining cross-laptop authority-discovery, duplicate-map, approved-map/proposal split, and cross-process workspace-receipt gaps without claiming they are implemented | Implement `CAD-WSP-PLAN-v1` M0–M3 before enabling portable discovery; then M4–M6 behind exact-object, UI-host, office-network, and platform evidence. Separately complete FOS AC-035 and certify the external M5 adapters | [Operator guide](FAST-ONBOARDING-AND-GIT-PERFORMANCE.md), [FOS implementation plan](FOS-IMPLEMENTATION-PLAN.md), and [capability/workspace extension plan](CAPABILITY-AUTHORITY-DISCOVERY-AND-WORKSPACE-PERFORMANCE-PLAN.md) |
