...
} = await import(source('views/submission-presentation.ts'));
...
function storySnapshot({ status = 'in_progress', generation = 0 } = {}) {
  ...
  return {
    ...
    documents: [artifact],
    submissionReadiness: {
      phaseId: 'design', phaseStatus: status, currentGeneration: generation,
      lifecycleReady: status === 'in_progress' && generation > 0,
      draftExists: true, draftModified: false,
      publicationRecorded: generation > 0,
      publishedGeneration: generation > 0 ? generation : null,
      classification: generation > 0 ? 'ready-to-attempt' : 'generation-required',
      reasonCode: generation > 0 ? 'SUBMISSION_READY' : 'PHASE_GENERATION_REQUIRED',
      nextSkill: generation > 0 ? '/sf-submit' : '/sf-phase',
      nextCommand: generation > 0
...
test('a checked-out Story exposes only engine-authorized generation or submission actions', () => {
  ...
  assert.equal(find(tree, 'story:design:prepare'), undefined,
    'preparation is not executed directly from the lifecycle tree');
  assert.equal(find(tree, 'story:design:publish'), undefined,
    'publication is not executed directly from the lifecycle tree');
  ...
  assert.deepEqual(find(tree, 'story:design:submit').command,
...
test('a seeded Story draft is not called generated and pre-fills its engine-selected skill', () => {
  const tree = buildTree(storySnapshot());
  const draft = find(tree, 'story-document:design:PHASE-DESIGN');
  assert.equal(draft.description, 'Seeded draft — not published');
  const generated = find(tree, 'story:STORY-42:generated-artifacts');
  assert.equal(generated.description, '0');
  assert.equal(find(generated.children, 'story-document:design:PHASE-DESIGN'), undefined,
    'generation-0 templates do not enter the generated artifact catalog');
  assert.equal(find(tree, 'story:design:submit'), undefined,
    'a positive generation or file presence is never inferred as submit permission');
  const action = find(tree, 'story:design:generate');
  assert.equal(action.label, 'Generate and publish Design');
  assert.equal(action.runCommand, 'singularityFlow.prefillStoryPhaseGeneration');
  assert.equal(action.prefill, '/sf-phase');

  const code = storySnapshot();
  code.submissionReadiness.nextSkill = '/sf-code';
  const codeAction = find(buildTree(code), 'story:design:generate');
  assert.equal(codeAction.prefill, '/sf-code');
  assert.equal(codeAction.description, 'prefill /sf-code for your review');

  assert.deepEqual(phaseGenerationChatPrefill(action.prefill),
    { query: '/sf-phase ', isPartialQuery: true });
  assert.deepEqual(phaseGenerationChatPrefill(codeAction.prefill),
    { query: '/sf-code ', isPartialQuery: true });
  assert.deepEqual(phaseGenerationChatPrefill('/sf-converge'),
    { query: '/sf-converge ', isPartialQuery: true });
  assert.equal(phaseGenerationChatPrefill('/sf-phase now'), null,
    'the prefill boundary accepts one exact SFlow skill, never appended instructions');
  assert.equal(submissionCommandArgv(storySnapshot().submissionReadiness, 'design'), null,
    'generation-required readiness cannot be parsed as a submission mutation');
});

test('a ready convergence action preserves the engine-owned advancement command', () => {
  const snapshot = storySnapshot({ generation: 1 });
  snapshot.workflow.currentPhase = 'design';
  snapshot.workflow.phases.design.id = 'design';
  snapshot.workflow.phases.design.label = 'Convergence';
  snapshot.submissionReadiness.phaseId = 'design';
  snapshot.submissionReadiness.classification = 'convergence-advance-required';
  snapshot.submissionReadiness.nextCommand = 'singularity-flow story advance --work-id STORY-42';
  const action = find(buildTree(snapshot), 'story:design:submit');
  assert.deepEqual(action.command, ['story', 'advance', '--work-id', 'STORY-42']);
  assert.deepEqual(submissionCommandArgv(snapshot.submissionReadiness, 'design'),
    ['story', 'advance', '--work-id', 'STORY-42']);
});

test('a soft gate never claims an unpublished generation was published', () => {
  ...
  Object.assign(snapshot.submissionReadiness, {
    ...
    nextSkill: '/sf-submit',
    nextCommand: 'singularity-flow submit design --work-id STORY-42'
  });
  const tree = buildTree(snapshot);
  assert.equal(find(tree, 'story:design:publication-status').label,
    'Seeded draft — not published; confirmation required');
  assert.doesNotMatch(find(tree, 'story:design:publication-status').label, /Published generation/);
  assert.deepEqual(find(tree, 'story:design:submit').command,
    ['submit', 'design', '--work-id', 'STORY-42']);
});

test('a recorded publication awaiting synchronization stays visible but not submittable', () => {
  const snapshot = storySnapshot({ generation: 1 });
  Object.assign(snapshot.submissionReadiness, {
  ...
  const tree = buildTree(snapshot);
  assert.equal(find(tree, 'story:design:submit'), undefined);
  assert.equal(find(tree, 'story:design:readiness-unavailable').label,
    'Published generation 1 — synchronization required');
  assert.equal(find(tree, 'story:design:continue-safely').runCommand,
...
test('missing readiness fails closed but keeps a safe way to ask the engine what is legal', () => {
  ...
  const tree = buildTree(snapshot);
  assert.equal(find(tree, 'story:design:submit'), undefined);
  assert.equal(find(tree, 'story:design:generate'), undefined);
  assert.equal(find(tree, 'story:design:readiness-unavailable').label, 'Submit is unavailable');
  assert.equal(find(tree, 'story:design:continue-safely').runCommand,
    'singularityFlow.continueSafely');

  const incomplete = storySnapshot({ generation: 1 });
  incomplete.submissionReadiness.nextCommand = null;
  const incompleteTree = buildTree(incomplete);
  assert.equal(find(incompleteTree, 'story:design:submit'), undefined,
    'readiness without its exact route cannot enable a mutation');
  assert.equal(find(incompleteTree, 'story:design:continue-safely').runCommand,
    'singularityFlow.continueSafely');
});
...
test('the Work Journey uses explicit readiness for generation and submission presentation', () => {
  ...
  const seeded = buildJourney(story);
  assert.equal(seeded.currentStage.publicationLabel, 'Seeded draft — not published');
  assert.equal(seeded.currentStage.artifacts[0].status, 'Seeded draft — not published');
...
test('the business inbox joins decisions to every generated artifact without listing empty templates', () => {
  ...
  shot.documents = [{
    ...
    id: 'PHASE-SEEDED', type: 'artifact', label: 'Seeded specification', kind: 'markdown',
    path: 'singularity/work-items/WORK-123/artifacts/specification/spec.md',
    phase: 'specification', status: 'draft', generation: 0, sha256: 'c'.repeat(64)
  }];
  ...
  assert.equal(inbox.artifacts.some((artifact) => artifact.label === 'Seeded specification'), false,
    'a hashable generation-0 seed is still a draft, never a generated artifact');
