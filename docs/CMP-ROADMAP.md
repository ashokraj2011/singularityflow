...
- Cause kinds, relationship kinds, dispositions, assurance classes, availability statuses,
  refusal codes, and diagnostic codes are closed vocabularies. Production diagnostic literals are
...
- no symbol-level, semantic, dependency-edge, or AST-derived completeness is claimed; cached symbol
...
isolated artifact, and runs the experimental `comprehension record-preview` against a separate Git
fixture while proving its status is unchanged. Until the same deterministic corpus passes the
supported platform matrix, treat this as an in-progress implementation tranche, not a shipped
product claim.
...
The P1 record-mode experiment landed at `main@da6338ab`. `comprehension record-preview
--experimental` emits only exact subject digests, aggregate counts, and closed reason classes. It
contains no source, path, Story, identity, prompt, or prose, invokes neither AST nor a model, writes
nothing, and is permanently labelled `authoritative: false`, `lifecycleGate: false`, and
`unverified-observation`. Its central migration-registry prototype reads sealed v1/v2 transport
bytes in memory, rejects tampering and unknown future versions, preserves the input file byte for
byte, and has no durable path or writer. This closes the code-local preview/migration deliverable;
...
again. Narrative-only drift stays separate from factual claims, while Candidate, claim, region,
cause-graph, structure, evidence, policy, and extractor dependency changes invalidate only the
claims that declare those dependencies. Removed claims and output/source ceilings remain explicit.
This is a transient diagnostic projection, not the durable governed revalidation receipt required
for P4 exit.

A deterministic zero-model draft path landed at `main@6f0227d9`. `comprehension walkthrough
draft` emits one exact resource-level `file-changed` claim per current change region and binds the
current Candidate compatibility subject, region manifest, and cause graph. It writes no file and
...
- the current release-gate smokes import the CMP contract from the installed npm package and the
  extracted VSIX engine, execute the model-free experimental record preview, and verify no Git
  status change.
...
Comprehension Center slice landed later at `main@eb88b989`, and its bounded exact Diff tab plus
portable release contract landed through `main@34e9b7c1`; neither that read-only surface nor the
earlier evidence satisfies the CMP v1 release criteria or any enforcement acceptance criterion.

Evidence at `main@da6338ab`: 29 focused command/migration/registry tests and 62 portable CMP/WEL
tests passed; the product check passed 1,367 checks; isolated npm and exact VSIX-contained engine
...
| `CMP-P4-001` | The bounded observe-only typed validator landed at `3fcaffea`; selective current-validator revalidation landed at `67a30f12`; deterministic zero-model resource drafting landed at `6f0227d9`; Candidate-bound bounded exact resource-source expansion landed at `e8bcbcce`. Add the optional model-draft transport, authoritative structural/evidence/human resolvers, authority-backed dependency resolution, and durable revalidation receipts | Current deterministic-draft, exact-source path/symlink/traversal/binary/bounds, self-awarded-assurance, malformed/oversized input/output, Candidate/dependency drift, counterfeit-prior-result, selective graph invalidation, dual-hash, circular-input, no-model, no-AST, no-write, and no-gate tests are green; counterfeit-model, prompt-injection, cancellation/timeout, authority-backed evidence/structure drift, and durable receipt evidence remain |
...
released when the panel is hidden or disposed. Normal activation snapshots omit the slice, and
...
- leased `comprehension` snapshot slice and cause-grouped review panel (**initial read-only form
  implemented**);
- rich per-region source expansion, clause/file/available-symbol navigation, replay timeline, unknowns,
  stale claims, and repair actions;
- `/sf-learn` guarded routing to signed-Pack lessons in disposable examples, never the live
  governed repository (**routing implemented; reviewed CMP lesson content remains**);
...
singularity-flow comprehension source <SFREF>
  [--offset BYTES] [--max-bytes BYTES] [--base REVISION] [--json]
