...
function sourceSection(view: VisualAssuranceView): string {
  ...
  return `<section class="plain assurance-section">
    ...
    ${view.candidates.length ? `<div class="notice warning"><strong>${view.candidates.length} newer candidate version${view.candidates.length === 1 ? '' : 's'} detected.</strong><p>A candidate becomes governed only through a new capture-phase generation and approval; this page does not silently replace the pin.</p></div>` : ''}
...
function comparisonSection(view: VisualAssuranceView): string {
  ...
  return `<section class="plain assurance-section"><div class="section-heading"><div><p class="eyebrow">Spec to screen</p><h2>${icon('compare')}Visual comparisons</h2></div><span class="pill">${view.comparisons.length} recorded</span></div>
    ...
    ${view.comparisons.length ? `<div class="comparison-grid">${view.comparisons.map((row) => `<article class="comparison-card ${statusClass(row.status)}">
...
export function visualAssuranceHtml(view: VisualAssuranceView, notice: string | null, operationError: string | null): string {
  ...
  return `<header class="inbox-header assurance-header"><div><div class="brand-lockup">SINGULARITY <span>FLOW</span></div><p class="eyebrow">Governed design review</p><h1>${icon('visual', { size: 24 })}Visual Assurance · ${escape(view.workId)}</h1><p class="meta">Pinned design evidence, implementation captures, deterministic comparisons, and MCP provenance in one review surface.</p></div>
    ...
    ${sourceSection(view)}${coverageSection(view)}${comparisonSection(view)}${mcpSection(view)}`;
}
