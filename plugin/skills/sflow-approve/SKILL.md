---
name: sflow-approve
description: Review and approve a submitted Singularity Flow phase as the current Git identity, recording its matched human authority group, optional working lens, self-approval warning, hashes, commit, and push.
...
The current Git identity must match one of the phase's configured approval authority groups. The working lens changes Copilot's review perspective only; selecting Architect, QA, or another lens never grants approval permission.
...
3. Before asking for a lens, show the reviewer identity and matched authority group returned by the CLI. If the identity is unauthorized, stop; do not offer a lens as a workaround.
4. When the CLI prints `Choose working lens`, call Copilot's `ask_user` tool with every displayed lens label, ID, and description as selectable options. Never infer or preselect one.
...
9. Report the decision commit, reviewer identity, authority group, working lens, identity assurance, self-approval status, remaining approvals, and next phase. Do not merge, deploy, or modify Jira.
...
2. Show the receipt's reviewer identity and authority result. Present the returned `persona` values as working-lens options through `ask_user`, then record only the reviewer's exact selected lens ID with `singularity-flow choices answer <TOKEN> persona <LENS-ID> --json`.
