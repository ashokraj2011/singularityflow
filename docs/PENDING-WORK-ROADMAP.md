...
- **Guarded activation work:** REV's manual built-in Candidate refinement pilot and fail-closed
  ...
  the trusted-editor, approved-runner, governed cross-clone receipt, and cross-platform witness
  gates pass. WEL now has a code-local structural Candidate/Program/attempt/approval/publication
  join and a partial SGOS Process/Program owner-verification token. `lifecycleJoined` remains false
  because approval-owner and complete cross-authority verification are absent; authenticated
  execution and enforcement remain unavailable until CAB/SGOS authority and external release gates
  are satisfied.
