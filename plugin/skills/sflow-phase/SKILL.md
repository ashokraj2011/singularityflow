...
10. Run `singularity-flow phase show <phase> --json`; show bounded evidence and report commit/push, resolved model, and token/cost status. For code or source evidence, show only a bounded source preview plus hash-bound references; never reproduce source files in full; never submit or approve. Say **Published generation <N> — ready to submit**; never say `publish-ready`. End with `Next in Copilot: /sf-submit <phase>`, followed by `Terminal equivalent: singularity-flow submit <phase>`.
