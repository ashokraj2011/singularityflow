...
For an auditable discovery result, use `singularity-flow session candidates --json --diagnostics` in a materialized repository. The legacy `--json` form remains an array; `--diagnostics` returns the candidate list plus fetch status and unreadable-branch details, so a failed or incomplete read is not mistaken for zero Stories. Before a delivery repository has a local checkout, use `singularity-flow session candidates --repository-url <DELIVERY-URL> --json --diagnostics`. If another repository owns its approved `sflow/config`, add `--configuration-url <LEAD-URL>`. URL-only discovery reads bounded Git metadata without creating an application checkout; it requires an accessible remote and a verified blobless fetch. It discovers published Story branches but does not attach them. Materialize or repair that repository in the workspace before `session attach`.
