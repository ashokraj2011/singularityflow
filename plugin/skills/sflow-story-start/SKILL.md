...
description: Select a Jira Story, workflow, and prompt-only working lens, then create or resume its canonical governed branch.
...
5. Start `singularity-flow story start <STORY-KEY> --fetch` in an interactive terminal and bridge the displayed workflow and working-lens choices through `ask_user`.
6. If persistent terminal input is unavailable:
   ...
   - Present the workflow-template and working-lens options with `ask_user`.
...
7. Show the resulting Epic → Jira Story → canonical branch lineage, selected workflow, working lens, current phase, generated intake document paths, commit, and push result.
