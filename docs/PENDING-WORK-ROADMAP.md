...
| Fast onboarding and safe Git | M0–M5 code-local contracts now exist: typed epoch-labelled queries/context, verified attach/refresh with bounded authority-move handling and journal recovery, package-approved local-only bootstrap plus a fail-closed organizational provider boundary, bounded digest-bound offline snapshots with finite pinned-policy reuse, exact leased remote publication and interrupted replay reconciliation, publication-time authorization freshness, per-capability decisions, non-sticky authority observations, no-discovery/AST/model Story start, concurrent World-Model view preservation, sealed worktree/index inputs, full-dependency derived cache, exact-OID/non-sticky-negative object service, Git accelerators, optional UX contracts, fail-closed governed-adoption boundaries, platform-safe refusal commands, terminal/interruption timing, delayed-work accounting, an explicit `--no-cache` comparator, and an executable exact-witness inventory (49/50 represented); all optional features default off | Run and retain the controlled physical performance witness for AC-035, then collect the remaining cross-platform/office certification matrix; certify the live organizational bootstrap provider, migrate additional legacy high-count readers under shadow comparison, and certify real identity/notification/server-gate/workflow-import adapters before enabling M5 | [Operator guide](FAST-ONBOARDING-AND-GIT-PERFORMANCE.md) and [implementation plan](FOS-IMPLEMENTATION-PLAN.md) |
