...
5. Run `singularity-flow story references verify --work-id <WORK-ID> --json`. Use only returned repository-relative `localPath` values, read-only. Run only its missing-checkout materialization; stop on invalid/dirty rather than reset. Run `prepare` then `phase begin`; require open intent, exact adoption digest, saved buffers, and unchanged snapshot.
