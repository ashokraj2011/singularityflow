...
test('the headline carries the gate count, and it matches the checklist', () => {
  ...
  assert.equal(card.headline, 'Not ready to submit — 2 of 13 gates unmet, 9 not evaluated');

  // A readiness read that finds nothing wrong is not headlined as a problem.
  assert.equal(buildResultCard(blocked([])).headline, 'Here is what SFlow found — 9 of 13 gates not evaluated');
