...
3. Inspect model-free with `auto plan --story`, `show-plan`, `list`, `status|report|needs-you`, or `continue`. Run `auto compare <FLIGHT-ID> --study <ID> --json` only for a selected reviewed study; preserve refusals. Do not mix new-work inputs into Story inspection. `auto adopt --from-adhoc <AHS-ID> [options] --json` verifies confirmed effects and creates a model-free Plan without moving or relabelling bytes. Starting requires a typed packet hash.
...
9. Reports show contracts, events, Candidate, lineage, quality, and separately assured economics. Reports never claim comparative savings; `auto compare` is a separate content-free read bound to exact reviewed Impact evidence. Never invoke `auto flight-step`, infer tokens, expose prompts, rank people, waive authority, expand scope, merge, deploy, or infer a hash. On failure, report references, Git outcome, checkpoint, and one exact next command.
