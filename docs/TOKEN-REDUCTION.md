# Token Reduction candidate preview
...
Developers may exercise `tkr-v1` only through the code-local token-reduction prompt adapter. That
API preserves section bytes and returns a candidate composition/report in memory; it does not
publish a Story packet, replace phase prompt bytes, write a grounding receipt, or dispatch a model
request. It requires a synchronous owner resolver that returns a closed, self-hashed binding; the
...
| M2 — WMP lifecycle integration | **Not implemented; blocked on the persisted WMP lifecycle** | Production use requires successor grounding/composition bindings, exact retained input delivery, expansion succession, freshness checks, handoff closure, request-boundary evidence, and restart/replay coverage. The missing semantic-owner and lifecycle work is tracked in [Persisted World-Model views](PERSISTED-WORLD-MODEL-VIEWS.md). |
...
1. Add an explicit shadow evaluator that compares a `tkr-v1` candidate while continuing to
   deliver and persist the exact `legacy-v1` bytes; a shadow failure must never block the Story.
2. Complete the missing persisted-WMP semantic owners and successor grounding/composition contract.
3. Bind the composer contract closure to the WMP successor composition and retained packet owner
   while preserving exact legacy replay.
4. Bind expansion and final outbound-delivery evidence to the exact successor composition.
5. Add owner-registered compact input renderers and lossless decoding, with golden, round-trip,
   limitations-overflow, clarification-state, and stable-segment assembly evidence.
6. Bind alias scope to the durable packet/composition owner; the preview currently enforces only
   the caller-supplied expected composition scope.
7. Run the declared paired benchmark and quality gates on representative Stories.
8. Consider changing a default only through governed configuration after the evidence is accepted.
