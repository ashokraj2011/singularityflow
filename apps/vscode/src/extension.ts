...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  */
  ...
  const registered: Record<string, (...args: never[]) => unknown> = {
    ...
    /**
     * "Talk to SFlow" is My Work. `[UXH:REQ-020]` `[UXH:D1]`
     *
     * There was a second home — `DeveloperHomePanel` — reading a `developer-home` envelope with its
     * own `context`, `choices[]` and `subjectRevision` at the top level. `sflow home` now answers in
     * `sflow-result` v2 and that projection rides inside `data.home`, so every field the panel
     * addressed came back undefined: a workspace of `undefined`, no choices, and a briefing headline
     * that was not there. It failed quietly, because reading a missing field is not an error.
     *
     * Kept as a command rather than removed. It is in the manifest, in the tree, and in the muscle
     * memory of anyone who has used this extension — retiring the panel is not a reason to retire
     * the door to it. The redirect is the whole implementation: one home, reached by both names.
     */
