...
| G1 — facade and reference transport | Partial. `src/git-access.mjs` now includes typed byte-preserving tree, refs, approved-config, metadata-only blob check, and exact-remote-ref observations, in addition to HEAD/ref, status/index, and exact raw blobs. A capability provenance path uses the facade in shadow mode while retaining its existing reference result. | Authorized-mutation descriptor contracts, more production read cutovers, and supported-platform qualification. Shadow mode is not an authoritative cutover. |
