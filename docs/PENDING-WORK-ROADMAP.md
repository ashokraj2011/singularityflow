...
| Fast onboarding and safe Git | M0–M5 code-local contracts now exist, including exact authority attachment/recovery, typed and cached Git reads, no-discovery/AST/model Story start, safe optional UX contracts, platform-safe refusals, terminal/interruption timing, delayed-work accounting, deterministic hashed 1k/10k/100k fixtures, exact-commit evidence registration, and an executable witness inventory (49/50 represented); the first immutable Darwin/arm64 controlled report passed every registered local budget and all optional features remain off | Complete AC-035 with controlled network, office, VS Code-host, Linux, and Windows evidence; then certify the live organizational bootstrap provider, migrate additional legacy high-count readers under shadow comparison, and certify real identity/notification/server-gate/workflow-import adapters before enabling M5 | [Operator guide](FAST-ONBOARDING-AND-GIT-PERFORMANCE.md) and [implementation plan](FOS-IMPLEMENTATION-PLAN.md) |
