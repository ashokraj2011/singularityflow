...
test('capability mapping reviews and activates the exact proposal instead of stopping at publication', async () => {
  ...
  const urlQuestion = content.indexOf('credential-free Git URL');
  const inspection = content.indexOf('capability inspect-repository <GIT-URL> --json');
  const details = content.indexOf('ask for ID, name, kind, parent, and scope');
  ...
  assert.match(content, /An unreachable repository is not new/i);
  assert.match(content, /On explicit mapping request/i);
  ...
  assert.match(content, /cancel-proposal <REVIEW-BRANCH>.*--confirm <FULL-COMMIT>.*--reason <REASON> --json/s);
  assert.match(content, /--supersede-branch <REVIEW-BRANCH> --supersede-commit <FULL-COMMIT>/);
  assert.match(content, /Deletion and creation must be atomic/);
  assert.match(content, /never cancel unrelated refs/i);
});
...
test('capability mapping previews and confirms the repository onboarding plan without inventing recovery', async () => {
  ...
  const confirmation = content.indexOf('returned `--confirm-plan` command once');
  assert.ok(preview >= 0 && confirmation > preview,
    'the skill previews and confirms onboarding in that order');
  assert.match(content, /Relay effects and preservation/i);
  assert.match(content, /only after confirmation/i);
  assert.match(content, /capability setup-proposal <SETUP-BRANCH> --lead <GIT-URL> --json/i);
  assert.match(content, /activate only after approval/i);
  assert.match(content, /Ask separately before `--acknowledge-unprotected`/i);
});
...
test('capability mapping onboards a selected team through one bounded atomic proposal', async () => {
  ...
  assert.match(content, /Inspect selected repos individually/i);
  ...
  assert.match(content, /Set problems aside/i);
  assert.match(content, /capability map-team <TEAM-ID>[\s\S]*--member <CHILD-ID>=<GIT-URL>/i);
  assert.match(content, /Show the proposed tree and command before mutation/i);
...
test('code explanation skill uses one read-only computed route and keeps narration explicit', async () => {
  ...
  assert.match(content, /Never read or summarize source files yourself/);
  assert.match(content, /perform any lifecycle mutation/);
});
