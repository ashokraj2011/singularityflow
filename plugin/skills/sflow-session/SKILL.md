...
8. From the same returned `repositoryPath`, run `singularity-flow nextsteps <WORK-ID> --json`, then report work item, remote commit, agent, phase, context revision/accounting, and those next steps. Stories always use `singularity-flow nextsteps`, never `singularity-flow initiative next`. Git identity remains the actor/approval principal.
