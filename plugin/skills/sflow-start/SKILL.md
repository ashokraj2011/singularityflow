---
name: sflow-start
description: Interactively choose Jira or manual intake, collect the story and documents, then select a configured workflow template and prompt-only working lens and publish durable Singularity Flow state.
...
- Start the Singularity Flow command in a persistent interactive shell. When it prints `Choose intake source`, `Choose workflow template`, or `Choose working lens`, call `ask_user` with the displayed labels, IDs, and descriptions as selectable options.
...
2. Present each returned `choiceSets` group through `ask_user`. When `--story-file` or other manual details were explicitly supplied, record `manual` for `intake-source`; when `--jira` was explicitly supplied, record `jira`. Still ask for the workflow template and working lens.
...
4. If `singularity/workflow.yml` is absent, run `singularity-flow init --work-id <WORK-ID> --base <BASE> --fetch`. This creates or reuses the Work-ID branch before writing configuration, so a protected base branch is never modified. Ask the user to review, commit, and push the generated workflow, templates, working-lens prompts, approval authorities, and builder prompt on the Work-ID branch before starting work. Do not require a direct push or merge to `main`.
...
7. For Jira intake, run `singularity-flow start <WORK-ID> --jira` through either interactive bridge. After Jira or manual intake is complete, collect the immutable workflow template and initial working lens from the contributor; never choose them for the user. Explain that the lens controls prompt perspective and does not represent the contributor's identity or authority.
