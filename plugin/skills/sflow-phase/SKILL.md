...
6. Run `singularity-flow phase publish <phase>`. Installer-managed Copilot telemetry is collected automatically and committed as a sanitized work-item record; add `--usage-json <file>` only for an exact external provider record. The generation commit must include the grounding composition record, prompt snapshot, and `telemetry/<phase>-gen<N>.json` without raw traces or conversation identifiers.
