...
function EpicStartWizard({ data, action, reload, generateWorldModel, openEpic, onSetupJira = () => window.dispatchEvent(new Event('singularity:setup-jira')) }) {
  ...
  const blankStarterAuthorities = Boolean(data.portfolio && authorityGroups.length
    && authorityGroups.every((authority) => !(authority?.members ?? []).length));
  const partiallyMissingAuthorities = Boolean(data.portfolio && !blankStarterAuthorities
    && authorityGroups.some((authority) => !(authority?.members ?? []).length));
  const needsPortfolioBootstrap = !data.portfolio || blankStarterAuthorities;
  ...
  const canStart = source === 'jira'
    ...
    : repositoryStartReady && !partiallyMissingAuthorities && localTitle.trim() && localDescription.trim() && localGoal.trim() && profile && persona && !localPreview?.error;
  return <div className="epic-start-wizard">
    ...
    <section className="panel epic-start-form">
      ...
      {partiallyMissingAuthorities && <div className="notice warn epic-start-blocker"><strong>Approval authority setup is incomplete.</strong><span>Open Configuration → Initiative governance and add at least one Git identity to every authority group used by this workflow.</span></div>}
