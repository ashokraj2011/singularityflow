...
5. Run `singularity-flow session status --json`, then report the selected persona, work-item scope, and whether it is bound to the current Copilot session. The CLI stores the persona under `.git/singularity-flow/session.json` and Copilot binding under `.git/singularity-flow/copilot-session.json`; opening or changing a session does not commit or push.
