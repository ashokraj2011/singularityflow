---
name: sflow-report
description: Generate a read-only Singularity Flow workflow performance report with phase duration, approval waiting, rework, token usage, optional configured cost, and bottlenecks.
...
3. Treat token totals as exact only where the provider supplied exact usage. Preserve `partial` and `unavailable` disclosures.
4. Show cost only when the workflow YAML contains pricing for the exact recorded model. Explain that configured prices are per million tokens and may be partial.
