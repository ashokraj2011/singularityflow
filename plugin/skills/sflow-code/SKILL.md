...
1. Require `ready` and exact `phaseAgent` readiness; run `singularity-flow phase show <phase> --json`. Require `generation.task: code`, never hard-code `implementation`, and otherwise stop with returned Copilot and Shell routes. Story context remains governed.
2. Before file access run `singularity-flow recover <WORK-ID> --phase <phase> --json`; execute its new-generation action and stop on manual/unchanged recovery.
...
5. Run `singularity-flow story references verify --work-id <WORK-ID> --json`; use returned `localPath` values read-only. Materialize only a missing checkout; stop on invalid/dirty. Run returned prepare/begin actions; require open intent, exact digest, saved buffers, and unchanged snapshot.
...
10. Run `singularity-flow phase show <phase> --json`; show bounded source preview, hash-bound references, commit/push, telemetry, tests, and submit routes. Never reproduce full sources; stop before submission/approval.
