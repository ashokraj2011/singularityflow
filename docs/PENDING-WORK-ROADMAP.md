...
| Fast onboarding and safe Git | M0–M5 code-local contracts now exist, including exact authority attachment/recovery, typed and cached Git reads, no-discovery/AST/model Story start, safe optional UX contracts, deterministic hashed 1k/10k/100k fixtures, exact-commit evidence registration, and an executable witness inventory (49/50 represented); the first immutable Darwin/arm64 controlled report passed every registered local budget, and M5 now rejects unverified/copyable adapter flags in favor of exact-runtime, externally attested identity/notification/server-gate/workflow-import bindings; all optional features remain off | Complete AC-035 with controlled network, office, VS Code-host, Linux, and Windows evidence; certify the live organizational bootstrap provider and the four real M5 adapters through the new boundary; migrate additional legacy high-count readers only under shadow comparison; then perform the independent enablement review | [Operator guide](FAST-ONBOARDING-AND-GIT-PERFORMANCE.md) and [implementation plan](FOS-IMPLEMENTATION-PLAN.md) |
