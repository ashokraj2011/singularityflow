...
1. Run `singularity-flow workspace current --json`. When it reports an active repository, run the progress command with the process working directory set to its exact `repositoryPath`; do not rely on a prior child command to have changed the shell directory. If no workspace is active, use the current governed repository and never search for one.
