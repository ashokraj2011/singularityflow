...
function pathGroundingContext(root, workId = null, workItemRoot = DEFAULT_WORK_ITEM_ROOT) {
  const governedDirectory = workId
    ? path.posix.dirname(workItemWorkflowRelative(workId, workItemRoot))
    : null;
  const workItem = workId
    ? ` Governed artifacts for ${workId} live under ${governedDirectory}/. Paths in Flow output are relative to one of these two roots.`
...
export async function copilotAgentStartHook(root, payload = {}) {
  ...
  if (status && ['local-only', 'ready'].includes(status.status)) {
    ...
    return {
      additionalContext: pathGroundingContext(
        root, session?.workId ?? null, definition?.workItemRoot ?? DEFAULT_WORK_ITEM_ROOT
      ).trim()
    };
  }
...
export async function sessionStartAgentHook(root, definition, workflow, payload = {}) {
  ...
  const workItemRoot = workflow?.resolution?.workItemRoot ?? definition.workItemRoot ?? DEFAULT_WORK_ITEM_ROOT;
...
export async function agentGuardHook(root, definition, workflow, payload = {}) {
  ...
  if (status.workItemSelectionRequired) {
    ...
    return {
      permissionDecision: 'deny',
      permissionDecisionReason: `Select and synchronize a Singularity Flow work/Jira ID before using '${tool}'. The contributor must choose the ID: ask them to type /sf-session, or to confirm the ID and then run '${attach}'. That command is permitted while this gate is active. Do not invoke the sf-session skill yourself; it is human-invoked only. Never infer the ID.`
    };
  }
  return {};
}
