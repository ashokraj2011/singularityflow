...
5. If the workspace contains multiple repositories, ask which repository to use. Clearly mark the lead repository when `singularity-flow workspace status` reports one — it is the repository the workspace's lead capability ships from, and it is where the orphan `state` branch lives.
