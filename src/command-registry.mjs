...
const LAZY_MODULES = Object.freeze({
  ...
  proof: './commands/proof.mjs',
  delivery: './commands/delivery.mjs',
...
const REVISION_MUTATION_SUBCOMMANDS = Object.freeze([
...
const COMPREHENSION_SUBCOMMANDS = Object.freeze([
  'regions', 'source', 'check', 'graph', 'explain', 'replay', 'walkthrough', 'brownfield',
  'backfill', 'record-preview'
]);
const DELIVERY_SUBCOMMANDS = Object.freeze([
  ...
  'provenance-status', 'local-runner-create', 'local-runner-status', 'local-runner-options', 'local-runner-plan',
...
export const RESOLVER_SUBCOMMANDS = Object.freeze({
  ...
  comprehension: COMPREHENSION_SUBCOMMANDS,
  delivery: DELIVERY_SUBCOMMANDS,
...
function resolveComprehensionOperation(definition, positionals) {
  ...
  if (subcommand === 'backfill') {
    const action = positionals[2] ?? 'validate';
    if (action !== 'validate') {
      return unknownSubcommand('comprehension backfill', action, ['validate'], 'action');
    }
...
function resolveProofOperation(definition, positionals) {
  const subcommand = positionals[1] ?? 'status';
  if (!['status', 'explain', 'gaps', 'signals'].includes(subcommand)) throw new SingularityFlowError(
    `'proof' has no subcommand '${subcommand}'.${didYouMean(subcommand, ['status', 'explain', 'gaps', 'signals'])}`,
    { code: 'UNKNOWN_SUBCOMMAND' }
  );
  return never(`proof.${subcommand}`, definition, 'read');
}

function resolveDeliveryOperation(definition, positionals) {
  const subcommand = positionals[1] ?? 'recommend';
...
function resolveStoryOperation(definition, positionals, options) {
  ...
  if (subcommand === 'references') {
    ...
    if (!STORY_REFERENCE_ACTIONS.includes(action)) {
      return unknownSubcommand('story references', action, STORY_REFERENCE_ACTIONS, 'action');
    }
    return never(`story.references.${action}`, definition, action === 'materialize' ? 'mutation' : 'read');
  }
...
function resolveRevisionOperation(definition, positionals, options) {
  const subcommand = positionals[1];
  if (REVISION_READ_SUBCOMMANDS.includes(subcommand)) {
    return never(`revision.${subcommand}`, definition, 'read');
  }
  if (REVISION_MUTATION_SUBCOMMANDS.includes(subcommand)) {
    if (subcommand === 'abandon' && optionBoolean(options, 'preview')) {
      return never('revision.abandon.preview', definition, 'read');
    }
    if (subcommand === 'capture' && optionBoolean(options, 'preview')) {
      return never('revision.capture.preview', definition, 'read');
    }
...
function resolveReviseOperation(definition, options) {
  return optionBoolean(options, 'dry-run')
    ? never('revise.preview', definition, 'read')
    : never('revise.apply', definition, 'mutation');
}
...
export function resolveOperation({ requestedCommand, positionals, options = {}, context = {} }) {
  ...
  if (definition.name === 'delivery') return resolveDeliveryOperation(definition, positionals);
  ...
  if (definition.name === 'revision') return resolveRevisionOperation(definition, positionals, options);
  if (definition.name === 'revise') return resolveReviseOperation(definition, options);
...
export function operationCatalog() {
  ...
  const reviseDefinition = commandDefinition('revise');
  ...
  const modelFreeMixed = [
    ...
    ...REVISION_MUTATION_SUBCOMMANDS.map((name) => never(`revision.${name}`, revisionDefinition, 'mutation')),
    never('revision.abandon.preview', revisionDefinition, 'read'),
    never('revision.capture.preview', revisionDefinition, 'read'),
    ...
    never('delivery.local-runner-plan', deliveryDefinition, 'read'),
    never('delivery.local-runner-run', deliveryDefinition, 'mutation'),
    never('delivery.local-runner-verify', deliveryDefinition, 'read'),
