---
name: sflow-session
description: Select a work or Jira ID, synchronize its latest committed remote branch, and bind an explicit prompt-only working lens before a Copilot session begins work.
...
5. Rerun `singularity-flow session status --json`. Only after `workItemSelectionRequired` is false may working-lens selection begin.
6. If `selectionRequired` is true, run `singularity-flow lens <WORK-ID>` in a persistent interactive shell. When the CLI prints `Choose working lens`, call `ask_user` with every displayed label, ID, and description. Never infer or silently reuse a lens when the policy requires a choice.
7. Map the contributor's selected lens ID to the displayed menu number and send only that number plus a newline to the same shell with `write_bash`.
