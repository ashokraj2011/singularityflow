...
6. Do not approve automatically. When the reviewer decides, use `/sflow-epic-review-decision` so the governed agent, rejection target, and exact packet confirmation are captured through a selection receipt. Approval authority still comes from the reviewer’s real Git/GitHub identity.
