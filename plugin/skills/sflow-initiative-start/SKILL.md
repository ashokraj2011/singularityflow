...
description: Start a multi-repository Singularity Flow initiative in GitHub Copilot by explicitly selecting an initiative profile and prompt-only session working lens.
...
3. Present every returned `choiceSets` group with Copilot's `ask_user`. Never infer, preselect, or silently default the initiative profile or working lens.
