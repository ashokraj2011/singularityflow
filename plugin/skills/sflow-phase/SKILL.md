...
5. Run `singularity-flow prepare <phase>`; read its template and approved inputs. Complete only configured artifacts, preserving metadata and clarifications. Re-read them; stop on `TODO`, `TBD`, unresolved `{{...}}`, or template instructions. Never publish an untouched template.
