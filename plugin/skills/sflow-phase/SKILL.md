...
10. Run `singularity-flow phase show <phase> --json`; show bounded evidence and report commit/push plus model/token/cost status. Never submit or approve. Say **Published generation <N> — ready to submit**, then show `/sf-submit <phase>` and `singularity-flow submit <phase>`.
