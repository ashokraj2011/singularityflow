...
export function workDir(root, config, id) { return path.join(root, config.workItemRoot ?? '.sdlc/work-items', id); }
export function workDirRelative(config, id) { return posix(path.join(config.workItemRoot ?? '.sdlc/work-items', id)); }
...
function ignored(config, workflow, relativePath) {
  if ([WORKFLOW_PATH, '.sdlc/config.json', '.sdlc/worldmodel.json'].includes(relativePath)) return true;
  if (relativePath.startsWith('.sdlc/world-model/')) return true;
...
export async function sourceTreeHash(root) {
  ...
  const files = [...new Set([...tracked, ...changedFiles(root)])].filter((file) => !file.startsWith('.sdlc/') && !file.startsWith('.git/') && !file.startsWith('node_modules/')).sort();
