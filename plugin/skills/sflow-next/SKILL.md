...
1. First run `singularity-flow session current --json`; require `ready`/`workId` and use its returned `repositoryPath` as cwd for every subsequent command. There run `singularity-flow nextsteps <WORK-ID> --json`; refuse `ACTIVE_SUBJECT_MISMATCH`. Use the shared repository model; `singularity-flow wm ensure` needs explicit consent. Never derive `--task`. Do not start it while waiting.
...
4. Follow the selected skill; never rewrite it to `/sf-phase`. If the selected action is `/sf-code`, do not imitate or inline it; report `Next in Copilot: /sf-code` and stop. Preserve `/sflow-code` likewise. The selected skill must draft-check and correct every agent finding. Never publish a delegated action.
...
8. After approval context `new`, use `/clear` then `/sf-next`; after `compact`, use `/compact` then `/sf-next`. Reapply the Boundary before reads.
