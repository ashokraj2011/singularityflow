...
const snapshot = {
  ...
  mcp: { servers: [{ id: 'figma', label: 'Figma', hostReference: 'figma', agents: [], phases: [], tools: ['get_file'], required: true, approval: 'confirm', configured: true, sources: ['workspace'] }], inventory: [], errors: [], warnings: [] },
  visualAssurance: {
    ...
    designSources: { approvedSet: { setSha256: 'a'.repeat(64), phase: 'design-intake', generation: 1, records: [{ recordId: 'design-1' }] }, candidates: [], stale: [], errors: [], warnings: [], passes: [] },
    ...
    comparisons: [{ id: 'visual-1', status: 'pass', profileId: 'mobile', differingPixels: 20, differingPixelRatio: .002, path: 'artifacts/visual-verification/evidence/visual-1.json', diffImage: { path: 'artifacts/visual-verification/evidence/visual-1-diff.png' } }],
...
test('visual assurance page exposes explicit review and network controls without auto-running them', () => {
  const html = visualAssuranceHtml(buildVisualAssuranceView(snapshot), null, null);
