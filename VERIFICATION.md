...
- Start, resume, approval, rejection, and persona skills use `ask_user` for YAML-derived selectable options and prefer `write_bash` to answer the same CLI picker; they never infer a default or pass hidden type/persona flags. Start and approval additionally support one-time, expiring, work-item/HEAD/session-bound selection receipts when persistent stdin is unavailable; approval also pins the phase, generation, artifact hashes, and exact typed phase confirmation.
