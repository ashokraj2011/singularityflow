...
CAS/replay/backup/restore/rollback journey now passes against both generic implementations and the
installed live implementation, with competing-writer, abandoned-lock, unfinished-stage, corrupt-
lineage, and forged-live-profile proofs. The current code-local continuation installs a separate `filesystem-live-v1`
...
remains partial only until signed supported-platform receipts are independently reviewed.
...
| REV code/test loop | The guarded built-in pilot now exposes preview/confirmation, private feedback attachments, exact parent/child Candidate retention, append-only interval lineage, manual IDE capture, deterministic precheck cards, recovery, and bounded Copilot/CLI reads. Classic Delivery Code generations can use it, while reviewer rejection still provides phase-level rework | Add the approved isolated executor, authenticated quality receipts, trusted unsaved-buffer adapter, exact selected-head publication bridge, binary scanning, and signed cross-platform witness matrix before autonomous/default activation | [REV delivery status](REV-DELIVERY-STATUS.md) |
...
- **Guarded activation work:** REV's manual built-in Candidate refinement pilot is active, but
  autonomous execution and ordinary selected-head publication remain disabled until its trusted-
  editor, isolated-execution, durable-receipt, and cross-platform witness gates pass. WEL
  Candidate/Program/attempt correlation likewise waits for
  CAB-R2 and SGOS-P0-001/CAB-R6; local observations keep those authority fields null.
...
6. qualify the guarded REV pilot, then complete its trusted-editor, isolated-executor, receipt,
   selected-head publication, and witness gates before autonomous/default activation;
