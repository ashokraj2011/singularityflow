...
- Opt-in Flow Impact studies with automatic Story enrollment, revision-bound receipts, honest exposure and missing-data records, aggregate privacy floors, uncertainty intervals, quality-gated inference labels, and deterministic two-arm comparisons of reviewed prompt sets.
...
The first clean reinstall of a legacy or unreceipted installation cannot promise automatic
restoration of old product bytes that were never retained. If that apply fails after removal, it
keeps the verified candidate bundle and prints one exact roll-forward recovery command. After a
schema-v2 receipt exists, normal source upgrades and promoted-distribution installs can admit exact
prior bytes and compensate touched surfaces automatically.
...
In Copilot, `/sf-help` loads the manual for general questions; `/sf-help WORK-123` loads the selected work item's immutable workflow guide. VS Code also provides the explicit, model-free `@sflow` participant. Its reviewed-help commands (`/help`, `/why`, `/how`, `/recover`, `/explain`, and `/topics`) share the packaged offline resolver, while `/next`, `/status`, `/checks`, `/converge`, `/docs`, `/inputs`, `/workflows`, `/approve`, and `/validate` render declared deterministic engine operations with zero model calls. The safe mappings are intentionally narrow: `/next` reads `nextsteps`, `/checks` uses `precheck --quick`, `/inputs` is an active-phase dry run, and `/approve` only reviews context before handing off to the existing guarded approval surface. Unknown free text shows command help; it is never model-routed. Drafting commands are not implemented. See [the `@sflow` Chat Participant boundary](docs/CPT-CHAT-PARTICIPANT.md).

Reviewed help cites the selected topic and opens guarded `/sf-*` actions as partial queries that the user must review and submit. VS Code includes the same resolver in the always-available **Help** view and searchable **Help Center**, bundled for offline use. It has focused entries for capabilities, workspaces, Story intake, workflow state, agents, prompts, world-model composition, troubleshooting, every `/sf-*` skill, and every top-level CLI command.
...
Build, test, package, and retained-artifact messages describe staging only; they do not mean that
the product was activated. Matching `singularity-flow --version` output is not sufficient either,
because distinct builds can share a semantic version. A full installation is complete only after
the installer verifies the exact installed `singularity-flow --build` provenance, verifies every
selected surface—including `singularity-flow plugin verify --json` when Copilot is installed—and
commits `~/.singularity-flow/installations/current.json`. Only then does it print the exact final
banner `Singularity Flow product activation — COMPLETE AND VERIFIED`. Explicitly narrowed modes
print `Singularity Flow product activation — PARTIAL BY REQUEST`; a normal install whose optional
manager is unavailable prints `Singularity Flow product activation — COMPLETE WITH SKIPS` and names
the skipped surface in its receipt. If no recognized final activation banner appears, use the
recovery command printed by the installer and do not infer success from earlier build/package output
or version labels. If activation committed but the process stopped before that banner, use the
printed receipt inspection and exact-build verification commands; do not blindly rerun installation.
...
Model-backed generation snapshots each UTF-8 prompt once into a private temporary file for exact
hashing, admission, optional consented capture, and cleanup. The verified text is delivered through
Copilot CLI's ACP stdio protocol; the temporary file is never attached to Copilot. Prompt content
never enters process arguments, environment variables, logs, telemetry, or invocation receipts;
receipts retain only its SHA-256, byte count, encoding, selected transport, and negotiated ACP
protocol version. The default input policy is 8 MiB. `singularity-flow doctor` blocks model-backed
generation when the configured Copilot executable does not advertise `--acp`. Provider transport
defaults to `auto`; `acp-stdio` forces a reviewed ACP wrapper and `attachment` is an explicit legacy
compatibility option. Provider configuration must not override adapter-owned cwd, prompt, ACP,
attachment, model, or tool-policy flags.
...
singularity-flow plugin verify --json (when Copilot is selected)
commit ~/.singularity-flow/installations/current.json
refresh approved configuration in registered workspace repositories
print Singularity Flow product activation — COMPLETE AND VERIFIED
```
...
into a content-addressed machine-local version store before activation; completing those build and
package steps only stages the candidate. The retained tarball is
...
the globally callable CLI last. Exact build provenance, rather than semantic-version equality alone,
binds that CLI verification. The committed installation receipt is the transaction commit point;
workspace configuration refresh remains a separate Git operation after activation.
...
Every new Story first requires an explicit branch published by every required repository; no branch is preselected, even when only one is available. With no source flags, `start` then asks whether intake comes from a Jira story or a manual description and documents. Manual mode asks for the title, audience, problem, outcome, acceptance criteria, and supporting file paths or HTTPS URLs. After source intake is complete, `start` asks for a workflow template (`feature`, `bugfix`, `chore`, `classic-delivery`, `figma-mobile`, `benchmarking-a`, `benchmarking-b`, `poc-lite`, `poc-workflow`, or another configured work type).

`classic-delivery` follows Intake → Code → Testing → Code checking. Code runs the configured structured repository tests and commits a normalized passing test receipt to the governed Story branch. Testing reviews those Code-phase results; Code checking verifies the same Git-bound evidence. Both refuse missing, failed, or stale receipts. Configure the repository's test adapter in the approved workflow before starting Code work. Fresh repositories receive the packaged work type at initialization; for an existing repository, preview `singularity-flow workflow install classic-delivery --dry-run`, then install it through its governed configuration path.
...
| `singularity-flow start <ID> --from-branch BRANCH [--jira \| --story-file FILE] [--work-type ID] [--ref BRANCH] [--isolated-worktree]` | Require an explicit published remote base, verify the configured remote before mutation, and create/push only the canonical Story branch. VS Code always creates a dedicated Story worktree; the CLI does the same automatically when its launch checkout is dirty, or explicitly with `--isolated-worktree`. The prior checkout, branch, index, and uncommitted files are not changed. Non-interactive callers must also pass `--work-type`; `--base` remains a standalone-repository compatibility alias. |
...
direct `sf-*` skill to be discovered and enabled at the expected source and
path. It always proves that installed direct aliases match this build byte for
byte; current path-bearing Copilot inventories also prove the packaged files.
