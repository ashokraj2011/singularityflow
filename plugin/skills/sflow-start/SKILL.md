...
1. Require a work ID. Use the opened root from `git rev-parse --show-toplevel`, else `singularity-flow workspace current --json`'s `repositoryPath`. Refuse if neither resolves; never search home/parents. There run `singularity-flow version` and `git status --short`; stop on missing CLI or dirt.
...
3. If `singularity/workflow.yml` is absent, run `singularity-flow init --work-id <WORK-ID> --base <BASE> --fetch`; it keeps protected `main` untouched. Stop for review/publication.
4. Run `singularity-flow workspace branches --json`. Require one branch published by every repository. Never infer or preselect. Stop if unreachable.
...
7. Without persistent input run `singularity-flow choices begin start <WORK-ID> --json`; present all choices including base and record via `singularity-flow choices answer <TOKEN>`. Add POC target separately. Use `--selection-receipt <TOKEN>` only when ready; it lasts 15 minutes and successful start consumes the receipt exactly once.
