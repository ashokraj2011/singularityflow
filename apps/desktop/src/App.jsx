...
import WorldModelExplorer from './world-model/WorldModelExplorer.jsx';
import {
  addWorldModelView,
  addPhaseToWorkType,
  createPersona,
...
function JiraWorkspace({ data, action, reload, onConfigure, bootstrapPortfolio, onDone }) {
  ...
  async function chooseEpic(epic) {
    ...
    const existing = data.initiatives.find((initiative) => initiative.id === epic.key);
    if (existing) {
      setInitiativeTarget('existing');
      setInitiativeId(existing.id);
    } else {
      setInitiativeTarget('new');
      setInitiativeId('');
    }
  ...
  async function adopt() {
    ...
    const result = await action(async () => {
      ...
      return {
        existing: false,
        adopted: await window.singularity.adoptJiraEpic(data.repository.root, targetId, selectedEpic.key, repositoryMap)
      };
    }, (outcome) => outcome.existing
      ? `${selectedEpic.key} already exists. Its latest remote branch was fetched and opened so you can continue.`
  ...
  const selectedExisting = selectedEpic
    ? data.initiatives.find((initiative) => initiative.id === selectedEpic.key) ?? null
    : null;
  async function continueExisting() {
    if (!selectedExisting) return;
    const opened = await action(
      () => window.singularity.openInitiative(data.repository.root, selectedExisting.id),
      `${selectedExisting.id} already exists. Its latest remote branch was fetched and opened.`
    );
    if (!opened) return;
    await reload(null, selectedExisting.id);
    onDone();
  }
...
function ApprovalInbox({ data, busy, refresh, attach }) {
  ...
  return <div className="page inbox-page">
    ...
    {!items.length ? <Empty title="Inbox clear" detail="No committed remote work-item phase is awaiting approval. Fetch the remote inbox to check for new submissions." /> : <section className="panel inbox-panel"><div className="inbox-header"><span>Work item</span><span>Phase</span><span>Approvals</span><span>Waiting</span><span>Authority groups</span><span /></div>{items.map((item) => <div className="inbox-row" key={`${item.id}:${item.phase}:${item.commit}`}><div><StatusDot status={item.status} /><span><strong>{item.id} — {item.title}</strong><small>{item.artifact ?? 'No required artifact'} · {item.commit?.slice(0, 8)}</small></span></div><span>{item.phaseLabel}<small>generation {item.generation}</small></span><span>{item.approvalsReceived}/{item.approvalsRequired}{item.selfApprovalWarning && <small className="warning-copy">self-approval</small>}</span><span>{item.waiting}</span><span>{item.reviewerAuthorities?.join(', ') || 'Any identified Git contributor'}</span><button className="secondary compact" onClick={() => attach(item.id)} disabled={busy}>Open review</button></div>)}</section>}
  </div>;
}
...
function PhaseGovernance({ data, selected, phaseId, action, reload }) {
  ...
  const contextBoundaryMode = selected.state.resolution?.contextPolicy?.phaseOverrides?.[phaseId]
    ?? selected.state.resolution?.contextPolicy?.onApproval
    ?? 'keep';
...
function EpicStartWizard({ data, action, reload, generateWorldModel, openEpic, onSetupJira = () => window.dispatchEvent(new Event('singularity:setup-jira')) }) {
  ...
  const authorityGroups = Object.values(data.portfolio?.approvalAuthorities ?? {});
  const authoritySetupIncomplete = Boolean(data.portfolio && authorityGroups.length
    && authorityGroups.some((authority) => !(authority?.members ?? []).length));
  const needsPortfolioBootstrap = !data.portfolio || authoritySetupIncomplete;
  const recoveringReservation = source === 'local'
    && localPreview?.recoverable
    && data.repository.branch === localPreview.id;
  const repositoryStartReady = (data.repository.branch === defaultBranch || recoveringReservation)
  ...
  return <div className="epic-start-wizard">
    ...
    <section className="panel epic-start-form">
      ...
      {recoveringReservation && <div className="notice good epic-start-setup"><strong>Reserved Epic {localPreview.id} is ready to resume.</strong><span>The earlier start stopped before initiative state was created. Singularity will reuse this committed reservation instead of allocating another Epic ID.</span></div>}
      <footer><div><strong>Next: source details → formatted requirements → Planning.</strong><span>The requirements specification and traceability file are committed to the Epic branch and become the approved inputs to Story planning.</span></div>{alreadyStarted ? <button className="primary" onClick={() => openEpic(alreadyStarted.id)}>Fetch & continue {alreadyStarted.id}</button> : <button className="primary" disabled={!canStart} onClick={start}>{configurationReadyToPublish ? recoveringReservation ? `Publish setup & resume ${localPreview.id}` : 'Publish setup & start Epic' : recoveringReservation ? `Resume ${localPreview.id}` : source === 'jira' ? 'Use Epic & start requirements' : 'Create requirements workspace'}</button>}</footer>
    </section>
  </div>;
}
  ...
  return <div className="split-page workflow-layout">
    <div className="design-pane"><header className="page-heading"><span className="eyebrow">Visual configuration</span><h1>Workflow designer</h1><p>Inspect phase order, approval authority, rejection paths, and template resolution.</p></header>
      ...
      <section className="gate-panel">
        <header><div><span className="eyebrow">Copilot session policy</span><h2>Identity and context boundaries</h2></div></header>
...
function Agents({ data, editor, setEditor, chooseAgent, saveEditor, createAgent, deleteFile, downloadFile, importAgent }) {
  ...
  return <div className="template-layout"><aside className="file-list"><header><div className="row-between"><div><span className="eyebrow">Prompt dependency registry</span><h2>Prompt packs</h2></div><div className="row"><button className="icon-button" title="Import prompt pack" onClick={importAgent}>⇧</button><button className="icon-button" title="Create prompt pack" onClick={() => setModal({ id: '', error: null })}>＋</button></div></div><p className="muted">These Markdown packs add skills, templates, or generated artifacts to Copilot context. They are not people and cannot approve.</p></header>{data.agents.map((agent) => <button key={`${agent.scope}:${agent.path}`} className={!lockView && current?.path === agent.path ? 'active' : ''} onClick={() => { setLockView(false); chooseAgent(agent); }}><span>PK</span><div><strong>{agent.id}</strong><small>{agent.scope} · {agent.remoteResources} remote</small></div></button>)}<button className={lockView ? 'active' : ''} onClick={() => setLockView(true)}><span>RO</span><div><strong>agents.lock.yml</strong><small>read-only trust state · refresh with CLI</small></div></button></aside>
