...
| G1 — facade and reference transport | Partial. `src/git-access.mjs` now includes typed byte-preserving tree, refs, approved-config, metadata-only blob check, and exact-remote-ref observations, in addition to HEAD/ref, status/index, and exact raw blobs. Capability provenance and optional Story status branch comparison use the facade in shadow mode while retaining their existing reference results. A default status-branch cutover was trialed and reverted: 30 local reads took 863–882 ms and 300 Git spawns through the typed path versus 88–90 ms and 30 spawns through the current path on this Mac. | Reduce repeated runtime/repository discovery safely before an authoritative status read cutover; then complete owner-scoped descriptor contracts, more production read cutovers, and supported-platform qualification. Shadow mode is not an authoritative cutover. |
...
discovery probes, and four unborn-HEAD probes. A dedicated, fully checked branch-only observation
could reduce this to two processes, but that path and its linked-worktree/deadline tests are not
implemented. Do not exchange a known tenfold latency regression for a nominal cutover.
