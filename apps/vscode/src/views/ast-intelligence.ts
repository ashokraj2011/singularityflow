...
import {
  astPolicyView, parseAstLanguageRows, parseAstPredicateRows, updateAstPolicyYaml,
  validateAstPolicyDraft, type AstAssurance, type AstFallback, type AstMode, type AstPolicyDraft
...
const SCRIPT = `
  ...
  document.addEventListener('click', (event) => {
    ...
    if (action) vscode.postMessage({ type: action.dataset.message, kind: action.dataset.kind });
  });
  document.addEventListener('submit', (event) => {
    event.preventDefault(); const form = event.target; const data = new FormData(form);
    if (form.id === 'ast-machine-form') vscode.postMessage({ type: 'save-machine', mode: data.get('mode') });
    if (form.id === 'ast-policy-form') vscode.postMessage({
    ...
    if (form.id === 'ast-scope-form') vscode.postMessage({
    ...
    if (form.id === 'ast-cache-confirm') vscode.postMessage({
...
function policyForm(policy: AstPolicyDraft): string {
  return `<section><div class="section-heading"><div><h2>${icon('worldModel')}Repository policy</h2><p>Saved through the governed configuration engine. Review and publish <code>singularity/workflow.yml</code> through the normal configuration path.</p></div><button class="secondary" data-message="open-configuration">Configuration Center</button></div>
    <form id="ast-policy-form">
      <div class="editor-card"><h3>Behavior</h3><div class="form-grid">
        <label><span>Repository mode</span><select name="mode">${option('auto', policy.mode, 'Auto — available when requested')}${option('off', policy.mode, 'Off — disable in this repository')}</select><small>Off is refused while any required predicate exists.</small></label>
...
export function astIntelligenceBody(policy: AstPolicyDraft, doctor: AstDoctorResult | null, run: AstRunResult | null, preview: AstCachePreview | null, notice: string | null, error: string | null): string {
  return `<header class="inbox-header"><p class="eyebrow">Configuration · World model</p><h1>${icon('worldModel', { size: 24 })}AST Intelligence</h1><p class="meta">Bounded structural facts, explicit assurance, and content-aware local caching. No daemon and no implicit whole-repository scan.</p></header>
    ...
    ${runtimeSummary(doctor)}${machinePreference(doctor)}${policyForm(policy)}${scopeRunner(policy)}${runResult(run)}${cacheSection(doctor, preview)}${adapterSection(doctor)}`;
}
...
export class AstIntelligencePanel {
  ...
  static repositoryChanged(): void { if (AstIntelligencePanel.current) { AstIntelligencePanel.current.doctor = null; AstIntelligencePanel.current.preview = null; AstIntelligencePanel.current.result = null; void AstIntelligencePanel.current.refresh(); } }
  private router = registerMessageRouter('singularityFlow.astIntelligence', {
    ...
    'open-workflow': () => { void vscode.commands.executeCommand('singularityFlow.openArtifact', { path: 'singularity/workflow.yml', label: 'workflow.yml' }); },
    'open-help': () => {
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      const doctor = commandData<AstDoctorResult>(await this.client.run(['wm', 'ast', 'doctor', '--json']));
      if (revision !== this.refreshRevision) return;
      this.doctor = doctor; this.error = null;
    } catch (error) { if (revision === this.refreshRevision) this.error = (error as Error).message; }
  ...
  private async savePolicy(message: InboundMessage): Promise<void> {
    ...
    try {
      ...
      this.notice = 'Repository AST policy saved locally. Review and publish the configuration when ready.'; this.error = null;
  ...
  private async executeCache(kind: 'prune' | 'clear', confirmation: string): Promise<void> {
  ...
  private render(): void {
    const token = nonce(); this.panel.webview.html = page('AST Intelligence', astIntelligenceBody(this.policy(), this.doctor, this.result, this.preview, this.notice, this.error), contentSecurityPolicy(this.panel.webview, token), token, SCRIPT, { nav: 'configuration' });
  }
}
