...
async function writePackageIndexes(root, config, workflow, manifest, packageRecord) {
  ...
  const gallery = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(packageRecord.name)} gallery</title><style>body{font:16px/1.5 Inter,system-ui,sans-serif;margin:32px;background:#f5f7f5;color:#17251d}h1{color:#16472b}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:18px}figure{margin:0;background:#fff;border:1px solid #d7dfda;border-radius:14px;overflow:hidden;box-shadow:0 8px 24px #183f2a12}img{display:block;width:100%;height:260px;object-fit:contain;background:#eef2ef}figcaption{display:flex;flex-direction:column;padding:14px}small{color:#68756d}</style></head><body><h1>${escapeHtml(packageRecord.name)}</h1><p>${images.length} image preview(s) from ${records.length} files. Open source files for full fidelity.</p><div class="grid">${cards || '<p>No image files were detected.</p>'}</div></body></html>`;
