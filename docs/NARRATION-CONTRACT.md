...
7. Review packet, pull-request description and continuation reuse — **done**: the recap reaches the
   pull-request body, `report --recap`, and the review bundle. It deliberately does **not** enter the
   review *packet*: `readStoryReviewPacket` hashes every field it reads back, so storing derived
   narrative there would put wording inside the evidence hash and let a copy-edit invalidate
   approvals. The packet holds evidence; the bundle renders it.
