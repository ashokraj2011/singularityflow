...
1. From the Boundary result require `ready` and exact `phaseAgent` readiness; run `singularity-flow phase show <phase> --json`. Require `generation.task: code`, never hard-code `implementation`, and otherwise stop with the returned Copilot and Shell routes. Story context stays in the governed workflow.
...
10. Run `singularity-flow phase show <phase> --json`; show bounded evidence, commit/push, telemetry, tests, and submission routes. Stop before submission/approval.
