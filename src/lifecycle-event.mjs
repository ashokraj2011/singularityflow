...
export function lifecycleEvent({
  ...
  if (!['story', 'initiative', 'adhoc', 'goal'].includes(subject?.kind) || !String(subject?.id ?? '').trim()) {
    throw new SingularityFlowError('Lifecycle events require subject.kind story|initiative|adhoc|goal and subject.id.');
  }
