...
const REVIEWABLE_INITIATIVE_OUTPUT_KINDS = new Set(['markdown', 'yaml', 'interface-contract']);

/** Human-authored Initiative/Epic outputs must be complete before any review can begin. */
async function initiativeOutputPlaceholderFindings(root, portfolio, initiative, phaseId) {
  const findings = [];
  for (const definition of phaseDefinition(initiative, phaseId).outputs) {
    // Generated projections are validated by their generator/schema. Scanning them as prose could
    // reject an exact projection merely because it faithfully reports a source-side TODO marker.
    if (definition.generator || !REVIEWABLE_INITIATIVE_OUTPUT_KINDS.has(definition.kind)) continue;
    ...
    for (const placeholder of artifactPlaceholderFindings(text)) findings.push({
      code: 'artifact.placeholder.unresolved',
      category: 'authoring',
      phase: phaseId,
      outputId: output.id,
...
function initiativePlaceholderMessage(finding) {
  return `output ${finding.phase}/${finding.outputId} (${finding.path}) contains unresolved placeholder '${finding.value}' at line ${finding.line}`;
}

/**
 * Read-only authoring health for an Initiative/Epic phase.
 *
 * This deliberately does not repair files. The producing host uses the returned findings to
 * correct its own draft in the current turn, while publication and approval retain their final
 * race-safe validation. Generated projections are listed but are regenerated by the kernel and
 * are never treated as agent-authored prose.
 */
export async function initiativePhaseDraftCheck(root, initiativeId, requestedPhase = null, {
  session = null
} = {}) {
  ...
  const outputs = [];
  const findings = await initiativeOutputPlaceholderFindings(root, portfolio, initiative, phaseId);
  const sessionAgent = typeof session?.agent === 'string' && session.agent.trim()
    ? session.agent
    : null;
  const sessionBound = Boolean(
    sessionAgent
    && session?.workId === initiative.initiative.id
    && session?.phaseId === phaseId
  );
  const targetGeneration = phase.status === 'in_progress'
  ...
  for (const outputDefinition of definition.outputs) {
    ...
    if (current.exists && REVIEWABLE_INITIATIVE_OUTPUT_KINDS.has(outputDefinition.kind)) {
      fingerprint = authoredArtifactFingerprint(await readFile(target.absolute, 'utf8'));
    }
    const reviewable = REVIEWABLE_INITIATIVE_OUTPUT_KINDS.has(outputDefinition.kind);
    const agentOwned = Boolean(
      !outputDefinition.generator
      && reviewable
      && sessionBound
      && output.generatedAgent === sessionAgent
      && Number(output.generation) === targetGeneration
    );
    const producer = outputDefinition.generator
      ? 'deterministic'
      : agentOwned
        ? 'governed-agent'
        : output.generatedBy && !output.generatedAgent
          ? 'human'
          : 'unknown';
    outputs.push(Object.freeze({
      ...
      required: initiativeOutputRequired(initiative, phaseId, outputDefinition),
      producer,
      ownershipProven: outputDefinition.generator || agentOwned,
      exists: current.exists,
      bytes: current.exists ? current.size : null,
      sha256: current.exists ? `sha256:${current.sha256}` : null,
      fingerprint
    }));
    if (initiativeOutputRequired(initiative, phaseId, outputDefinition) && !current.exists) {
      findings.push({
        code: 'artifact.required.missing', category: 'authoring', phase: phaseId,
        outputId: output.id, path: target.relative, line: null, value: null
      });
    }
  }
  ...
  const reviewableOutputs = outputs.filter((output) => REVIEWABLE_INITIATIVE_OUTPUT_KINDS.has(output.kind));
  const draftDigest = createHash('sha256');
  for (const output of reviewableOutputs) {
    draftDigest.update(`${output.path}\0${output.exists ? output.fingerprint : '<missing>'}\n`);
  }
  const findingProducers = findings.map((finding) =>
    outputs.find((output) => output.id === finding.outputId)?.producer ?? 'unknown');
  const agentMayCorrect = findings.length > 0
    && findingProducers.every((candidate) => candidate === 'governed-agent');
  const producerSet = new Set(reviewableOutputs
    .filter((output) => !definition.outputs.find((item) => item.id === output.id)?.generator)
    .map((output) => output.producer));
  const producer = producerSet.size === 1 ? [...producerSet][0] : 'unknown';
  const awaitingApproval = phase.status === 'awaiting_approval';
  const clean = findings.length === 0;
  const correctionClass = agentMayCorrect ? 'agent-authoring' : 'human-input';
  return Object.freeze({
    schemaVersion: 1, // schema-transient: read-only process projection, never persisted
    resultType: 'sflow-initiative-phase-draft-check',
    ...
    ownership: Object.freeze({
      proven: clean ? producer === 'governed-agent' : agentMayCorrect,
      sessionBound,
      agent: sessionAgent,
      reason: !sessionBound
        ? !session ? 'active-session-missing'
          : session?.workId !== initiative.initiative.id ? 'active-session-bound-to-different-work'
            : session?.phaseId !== phaseId ? 'active-session-bound-to-different-phase'
              : 'active-session-agent-missing'
        : (clean ? producer === 'governed-agent' : agentMayCorrect)
          ? 'active-session-and-output-provenance-match'
          : 'affected-output-ownership-unproven'
    }),
    draftFingerprint: `sha256:${draftDigest.digest('hex')}`,
    outputs: Object.freeze(outputs),
    findings: Object.freeze(findings.map((finding) => Object.freeze({
      ...finding,
      message: finding.code === 'artifact.required.missing'
        ? `Required output ${finding.phase}/${finding.outputId} is missing: ${finding.path}`
        : initiativePlaceholderMessage(finding)
    }))),
    correction: Object.freeze({
      class: correctionClass,
      automatic: false,
      sameTurn: agentMayCorrect && !awaitingApproval,
      requiresNewGeneration: awaitingApproval && !clean,
      maximumChangedFingerprints: 3,
      guidance: clean ? null : agentMayCorrect
        ? 'In the current Copilot turn, re-author every finding from approved Initiative evidence. Do not delete markers, invent facts, add padding, or launch a nested model.'
...
export async function evaluateInitiativePhase(root, portfolio, initiative, phaseId, { now = new Date() } = {}) {
  ...
  const authoringFindings = await initiativeOutputPlaceholderFindings(
    root, portfolio, initiative, phaseId
  );
  errors.push(...authoringFindings.map(initiativePlaceholderMessage));
...
export async function publishInitiativePhase(root, initiativeId, phaseId, { agent = null } = {}) {
  ...
  if (authoringFindings.length) {
    throw new SingularityFlowError(
      `Initiative phase '${phaseId}' is not publishable:\n- ${authoringFindings.map(initiativePlaceholderMessage).join('\n- ')}\n`
      + 'Correct the listed output artifact(s), then publish the phase again.',
      {
        code: 'ARTIFACT_AUTHORING_INCOMPLETE',
        details: {
          subjectKind: 'initiative', initiativeId: initiative.initiative.id, phase: phaseId,
          findings: authoringFindings
        }
      }
    );
  }
...
export async function approveInitiative(root, {
  ...
  const authoringFindings = await initiativeOutputPlaceholderFindings(
    root, portfolio, initiative, selectedPhase
  );
  if (authoringFindings.length) {
    throw new SingularityFlowError(
      `Initiative phase '${selectedPhase}' cannot be approved while outputs are incomplete:\n- ${authoringFindings.map(initiativePlaceholderMessage).join('\n- ')}\n`
      + 'Correct the listed output artifact(s), then publish and review a new generation.',
      {
        code: 'ARTIFACT_AUTHORING_INCOMPLETE',
        details: {
          subjectKind: 'initiative', initiativeId: initiative.initiative.id,
          phase: selectedPhase, findings: authoringFindings
        }
      }
    );
  }
