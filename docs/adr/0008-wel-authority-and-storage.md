...
| Static JUnit declaration identity | WEL JUnit adapter observation | `test-execution` receipt v4 (identity projection introduced in v3) |
| Clause-to-test proposal | WEL JUnit adapter observation | `test-execution` receipt v4, status `unreviewed` (proposal projection introduced in v3) |
...
mapping decision. A structural observe-only Candidate/Program/attempt/approval/publication join now
exists, but the pilot still reports `inconclusive` and `testcaseExecutionProven: false` because it
does not yet have an authenticated independent runner, a complete owner-verified cross-authority
join, or trusted toolchain attestation. Existing module-level Code Delivery evidence remains authoritative;
WEL neither upgrades nor blocks it.

## Compatibility and migration

`test-execution` v2 records migrate in memory through v3 to v4 with `exact: false`, no catalog, no
mapping proposals, and an explicit lifecycle-unavailable projection. The v4 projection cannot
manufacture Candidate, Program, retry, approval, publication, or authenticated-runner authority.
Historical bytes are never rewritten and historical observations never gain authority.
