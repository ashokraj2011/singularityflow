...
When the composed prompt contains **Human clarification checkpoint**, execute it before authoring the artifact. Use `ask_user` for the configured question batch and wait for the response. A `required` checkpoint always pauses at least once; when the evidence is complete, ask the contributor to confirm your concise interpretation rather than skipping the checkpoint. Do not substitute an “Open questions” section for the interactive checkpoint. Incorporate accepted answers into the governed artifact and keep only explicitly deferred decisions under Open questions. If `ask_user` is unavailable, show the numbered questions and stop before authoring or publication.
