...
singularity-flow session open-local WORK-123
```

`candidates` fetches the configured remote and lists only branches containing a valid workflow at the expected work-item path. `attach` verifies the exact selected remote Story and fast-forwards to its verified remote head. It reuses an existing managed Story worktree, creates one when invoked from another Story worktree, or may switch a clean canonical checkout for compatibility. An exact already-current branch at remote HEAD may instead be bound in place, preserving unpublished edits. If its selected workspace repository was registered without a checkout, attach first materializes only that repository. It never silently switches an unrelated Story worktree. A missing remote Story branch is never created implicitly; use `/sf-start` for new work. The command returns the actual attached `repositoryPath`; `cd` to that path in a terminal before running Story commands, because child processes cannot change the parent shell's cwd. Copilot `/sf-session` uses that path for subsequent commands, and VS Code opens it after successful attachment.

`open-local` is a separate choice for an existing, registered managed Story worktree. It proves the exact local Story and repository identity, activates its local phase session, and returns its checkout path without fetching, checking out, resetting, or changing Git files. It reports `localOnly: true` and `remoteSynchronized: false`; it does not claim to have the latest remote revision. It is safe for an ahead or dirty worktree. If no verified managed checkout exists, use `attach` for remote synchronization and materialization. VS Code Story selection tries this local route first, then remote attach only on a local miss; after opening an active Story checkout it starts a fresh governed Copilot chat.

For an auditable discovery result, use `singularity-flow session candidates --json --diagnostics`. The legacy `--json` form remains an array; `--diagnostics` returns the candidate list plus fetch status and unreadable-branch details, so a failed or incomplete read is not mistaken for zero Stories. To select a workspace repository even when the current directory is another Story's worktree, pass `--workspace <WORKSPACE> --repository <REPOSITORY-ID>` to `session candidates`, `session open-local`, or `session attach`. The selected repository may still be deferred: candidates uses bounded remote metadata and attach can materialize its checkout after exact Story verification. The `repositoryPath` in a candidates result is the scan source, not necessarily the Story checkout; use the verified path returned by `open-local` or `attach` as the destination. For a delivery URL outside a selected workspace, `session candidates --repository-url <DELIVERY-URL> --json --diagnostics` still offers read-only discovery. If another repository owns its approved `sflow/config`, add `--configuration-url <LEAD-URL>`. A URL-only candidate alone does not authorize checkout materialization or attachment; select an exact workspace/repository first.

Across laptops, only Story commits pushed to the configured remote can be discovered. An unpublished local draft or branch on the first laptop will not appear on the second. In VS Code, attach the mapped capability to a workspace (or create a workspace from it) to include its Stories in that workspace's Inbox; selecting the workspace starts discovery, and **Inbox → Refresh Stories** repeats it. Mapping alone does not automatically attach a capability to a workspace. A repository failure or unreadable branch is reported as incomplete coverage, with the other repositories' verified Stories retained. Repair Git access or the reported repository configuration and retry; do not create a replacement Story because discovery was incomplete.

Ahead, diverged, missing, malformed, or unverifiable state stops without history rewriting or data loss. A dirty tree also stops when attachment would require a checkout or fast-forward. When the requested Story branch is already current and its HEAD exactly matches the freshly fetched remote HEAD, Singularity Flow may bind the Copilot session in place while preserving unpublished phase edits. If a pre-existing local work branch is ahead or diverged, it remains untouched so the contributor can preserve or publish it. Singularity Flow never merges, rebases, resets, stashes, force-checks out, or deletes work during session attachment. An opened clone or an exact selected workspace/repository supplies the Git remote; the user's current directory alone never justifies choosing another repository.
...
Selecting an active Story in the workspace Inbox first opens a verified existing managed checkout, preserving local ahead or dirty work without claiming a remote sync. If no local checkout exists, strict remote attachment materializes only its selected deferred repository, then reuses or creates a managed Story worktree or switches a clean canonical checkout. That remote path must fast-forward exactly to the fetched commit; ahead, diverged, malformed, or missing states stop without merging, rebasing, resetting, stashing, or discarding work. VS Code opens the returned `repositoryPath` and starts a fresh governed Copilot chat for the selected Story. The phase agent activates automatically; approval and rejection remain separate reviewed actions.
...
singularity-flow local start <NAME> --intent <TEXT> --input <ABSOLUTE-PATH>... --classification public|internal|confidential|restricted [--allow-empty-output] [--json]
singularity-flow local list [--json]
singularity-flow local status --story <LOC-ID> [--json]
singularity-flow local freeze --story <LOC-ID> [--output <ABSOLUTE-PATH>]... [--json]
singularity-flow local verify --story <LOC-ID> --candidate <SHA256> [--json]
singularity-flow local signer-create --story <LOC-ID> --signer <KEY-ID> [--json]
singularity-flow local trust-export --story <LOC-ID> --signer <KEY-ID> --out <ABSOLUTE-PUBLIC-KEY-FILE> [--json]
...
sflow-agent [WORK-ID]
singularity-flow session status|open-local|attach|candidates|workspace [WORK-ID] [--json]
singularity-flow auto plan "<REQUIREMENT>" [--capability ID] [--work-type ID] [--work-id ID] [--from-branch BRANCH] [--profile story|auto-select|sgos] [--pace PACE] [--until STOP] [--json]
...
singularity-flow auto show-plan <PLAN-ID> [--json]
singularity-flow auto start <PLAN-ID> --confirm <PACKET-SHA256> [--json]
singularity-flow auto start --plan <PLAN-ID> --confirm <PACKET-SHA256> [--json]
singularity-flow auto list [--json]
singularity-flow auto continue <STORY-ID> [--json]
singularity-flow auto adopt --from-adhoc <AHS-ID> [--json]
singularity-flow auto status|report <FLIGHT-ID> [--json]
singularity-flow auto compare <FLIGHT-ID> --study <IMPACT-STUDY-ID> [--json]
singularity-flow auto needs-you <FLIGHT-ID> [--json]
singularity-flow auto repair <FLIGHT-ID> --refusal <REFUSAL-ID> [--confirm <REPAIR-PLAN-SHA256>] [--json]
singularity-flow auto respond <FLIGHT-ID> --request <REQUEST-ID> (--choice <ID> | --answer <TEXT> | --broker-reference <REFERENCE>) --confirm <REQUEST-SHA256> [--json]
singularity-flow auto switch-unit <FLIGHT-ID> --execution-unit <ID> [--reason <TEXT>] [--confirm <SWITCH-PLAN-SHA256>] [--json]
singularity-flow auto pause|stop|halt|takeover <FLIGHT-ID> [--confirm <CHECKPOINT-SHA256>] [--json]
...
singularity-flow adhoc start ["NOTE"] [--from REVISION] [--include-existing] [--mode in-place] [--json]
singularity-flow adhoc status [SESSION-ID|all] [--json]
singularity-flow adhoc diff|effects|evidence [SESSION-ID] [--json]
singularity-flow adhoc pause|resume [SESSION-ID] [--json]
singularity-flow land [SESSION-ID] [--json]
singularity-flow adhoc intent confirm [SESSION-ID] --objective TEXT --success TEXT --confirm CHANGE-SET-SHA256
singularity-flow adhoc claim <RESOURCE>|--all --clause CLAUSE-ID [--session SESSION-ID]
...
singularity-flow learn bundle-create LESSON-ID --role ROLE --module LEARNING-MODULE.json --fixture LEARNING-FIXTURE.json [--pack PACK-ID] --trust PUBLIC-TRUST.json --out NEW-BUNDLE.json [--json]
singularity-flow learn bundle-inspect --bundle BUNDLE.json [--json]
singularity-flow learn bundle-materialize --bundle BUNDLE.json --trust PUBLIC-TRUST.json [--confirm PLAN-SHA256] [--json]
singularity-flow learn workspace MISSION-SHA256 [--json]
singularity-flow learn progress|progress-export MISSION-SHA256 [--json]
singularity-flow learn check LESSON-ID CHECK-ID --role ROLE --module LEARNING-MODULE.json --answers ANSWER.json [--pack PACK-ID] --trust PUBLIC-TRUST.json [--json]
singularity-flow learn progress-import --transfer IDENTITY-FREE-TOKEN [--confirm PLAN-SHA256] [--json]
...
singularity-flow comprehension walkthrough revalidate <DRAFT-FILE> <PREVIOUS-VALIDATION-FILE>
  ...
  walkthrough operations are deterministic, read-only, model-free diagnostics and never grant authority
