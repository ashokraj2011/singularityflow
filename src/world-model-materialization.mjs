...
async function inspectCandidate(root, config, plan, candidate, sourceState) {
  ...
  try {
    ...
    if (!missing.length) {
      ...
      return { ...candidate, ready: freshness.fresh, fresh: freshness.fresh, selections, manifest: validated.normalizedManifest, sourceTreeSha256: freshness.built, error: freshness.fresh ? null : `source snapshot differs (${freshness.built} != ${sourceState.sha256})` };
    }
    return { ...candidate, ready: false, fresh: false, selections, manifest: normalized, sourceTreeSha256: normalized.source_tree_sha256 ?? null, error: `missing ${missing.map((entry) => entry.id).join(', ')}` };
  } catch (error) {
    return { ...candidate, ready: false, fresh: false, selections: [], manifest: normalized, sourceTreeSha256: normalized?.source_tree_sha256 ?? null, error: error.message };
  }
}
...
export async function inspectGroundingAvailability(root, config, plan) {
  ...
  const conflicts = policy.publish === 'governed' && worktreeReady && !legacyWorktreeReady && !governedReady
    ? [{
...
export async function mergeWorldModelSnapshot({ existingDirectory = null, fragmentDirectory, targetDirectory, plan, sourceTreeSha256, materialization }) {
  ...
  merged.domains = [...new Map([...(existing.domains ?? []), ...(fragment.domains ?? [])].map((entry) => [entry.id, entry])).values()];
  merged.task_guides = [...new Map([...(existing.task_guides ?? []), ...(fragment.task_guides ?? [])].map((entry) => [entry.id, entry])).values()];
