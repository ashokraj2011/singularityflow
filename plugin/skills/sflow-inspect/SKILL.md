---
name: sflow-inspect
description: Inspect the selected Story, progress, documents, evidence, prompt composition, comprehension diagnostics, and next actions without mutation.
disable-model-invocation: true
argument-hint: "[WORK-ID] [status|progress|documents|prompt|comprehension|next]"
---
...
2. Route the requested view to `progress`, `documents list`, `show-prompt`, `report`, `nextsteps`, or the read-only `comprehension regions|check` pilot. For comprehension, keep caller files repository-contained and describe them as untrusted diagnostics; never claim the compatibility subject is the universal Candidate. With no view, show status, progress, generated artifacts, approvals, warnings, and next actions.
