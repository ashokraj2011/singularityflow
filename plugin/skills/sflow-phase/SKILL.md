...
1. Run `singularity-flow status --json`; use only the active phase and current working-lens session.
2. Run `singularity-flow wm compose --phase <phase> --task "<work objective>"`, adding `--evidence` when configured, and use the complete returned prompt. If the model or exact task guide is missing or stale, first run `singularity-flow wm build --phase <phase> --task "<work objective>"`, then rerun the identical compose command. Composition records the working lens, mandatory phase/lens views, task guide, rule-selected files, evidence, prompt-pack skills, hashes, and exact prompt snapshot for the next generation.
