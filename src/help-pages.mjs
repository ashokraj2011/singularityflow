...
const PAGES = Object.freeze({
  ...
  comprehension: {
    ...
    description: [
      ...
      'enforcement remains unavailable until CMP binds the shipped universal Candidate through',
      'the existing Story review, approval, and publication transaction.',
      '',
      ...
      '',
      '`comprehension regions` returns Candidate-bound source references for the exact before and',
      'after bytes that are represented by each region. `comprehension source` expands one such',
      'reference in bounded binary-safe pages. Before bytes come from the immutable Git blob;',
      'after bytes use a no-follow working-tree read and must still equal the manifest digest.',
      'Missing, changed, oversized, escaped, or unsupported sources are refused. This exact read',
      'does not require AST or a world model and cannot grant lifecycle authority.',
      '',
    ...
    options: [
      ...
      ['source SFREF', 'Expand one exact Candidate-bound source reference returned by regions or walkthrough validation.'],
      ['--offset BYTES', 'For source, begin the bounded page at this byte offset.'],
      ['--max-bytes BYTES', 'For source, return at most this many bytes, up to the fixed 64 KiB page ceiling.'],
    ...
    examples: [
      ...
      ['singularity-flow comprehension source sfref:comprehension:source:<REGION>:after:<DIGEST> --max-bytes 16384 --json', 'Read one digest-checked binary-safe source page without invoking AST or a model.'],
  ...
  learn: {
    ...
    description: [
      ...
      'tutorial. If setup is interrupted before its manifest is published, `workspace` reports',
      '`interrupted` and the same exact confirmed `materialize` request resumes only matching bytes.',
      '`check` stores only passed check IDs. `progress-export` returns an explicit',
      ...
      'timing, paths, scores, approval, or certification. Current v2 progress reads and migrates',
      'canonical v1 records and tokens in memory; the next successful write upgrades local storage.',
      '`bundle-create` writes one bounded, self-hashed module/fixture bundle for explicit offline',
      'copy. `bundle-inspect` verifies those inert bytes without Pack credentials. A destination',
    ...
    options: [
      ...
      ['--bundle FILE', 'Read one bounded, self-hashed offline learning module/fixture bundle.'],
      ['--out FILE', 'Create one new repository-contained offline bundle file; never overwrite an existing path.'],
    ...
    examples: [
      ...
      ['singularity-flow learn bundle-create recovery-basics --role developer --module recovery.json --fixture recovery-fixture.json --trust publisher-trust.json --out recovery.bundle.json', 'Create one explicit offline module/fixture bundle bound to the active Pack.'],
      ['singularity-flow learn bundle-inspect --bundle recovery.bundle.json', 'Verify copied bundle bytes without granting or requiring Pack authority.'],
      ['singularity-flow learn bundle-materialize --bundle recovery.bundle.json --trust publisher-trust.json', 'Preview offline materialization against the exact matching local active Pack; repeat with its confirmation digest.'],
      ['singularity-flow learn workspace sha256:<MISSION-DIGEST>', 'Verify local tutorial files and diagnose an interrupted setup without Pack credentials.'],
  ...
  bootstrap: {
    ...
    options: [
      ...
      ['--grounding off|warn|enforce', 'World-model context policy. Missing intelligence never blocks. Enforce verifies hashes, provenance, and snapshots whenever context is consumed.'],
  ...
  revision: {
    ...
    description: [
      ...
      'Revision checks expose a separate Candidate-bound browser-check surface. Capabilities is machine-local; plan selects only the exact current retained Candidate and approved phase-registered browser checks; status and result inspect durable run records. Callers cannot supply a command, URL, adapter, environment, Candidate, phase, or check definition.',
      'A checks plan is read-only. Checks run requires the same full digest in --plan and --confirm and fails closed when no approved same-process runner can prove the fixed broker receipt. This slice exposes no public cancel or retry mutation. Browser observations never become repository-test, Testing/Verification, publication, approval, merge, deployment, or release authority.',
    ...
    examples: [
      ...
      ['singularity-flow revision checks capabilities --json', 'Inspect the installed browser-check runner and its evidence boundary without selecting a Story or running anything.'],
      ['singularity-flow revision checks plan --json', 'Plan approved browser checks against the exact current retained Candidate; do not execute them.'],
      ['singularity-flow revision checks status [<RUN-ID>] --json', 'Inspect the current or named durable browser-check run state.'],
      ['singularity-flow revision checks result <RUN-ID> --json', 'Read the named run receipt and comparison without inventing a pass or lifecycle authority.'],
      ['singularity-flow revision checks run --plan sha256:<PLAN> --confirm sha256:<PLAN> --json', 'Attempt only the exact confirmed plan through the installed fixed broker; refuse when an approved runner is unavailable.'],
  ...
  workspace: {
    ...
    description: [
      ...
      'probe the remotes referenced by unfinished bootstrap sessions. Add one or more exact,',
      'credential-free `--repository` URLs to diagnose a remote when no setup session exists.',
      'It reports only whether enterprise proxy and CA sources exist; it never prints their values',
      'or suggests disabling TLS.',
      '',
    ...
    options: [
      ...
      ['--clone', 'Materialize application checkouts during workspace setup; the default registers planned paths and defers cloning until work needs code.'],
      ['--initialize', 'Explicitly check out the lead and initialize its governed state branch; implies --clone unless --no-clone is given, which is refused.'],
      ['--network', 'Allow workspace doctor to contact pending-session remotes and exact URLs supplied with --repository.'],
      ...
      ['--repository VALUE', 'Repeatable. For doctor, an exact credential-free URL; for create/update, ID=URL; for repair or configuration refresh, a repository ID.'],
    ...
    examples: [
      ...
      ['singularity-flow workspace repair /workspaces/payments --repository payments-api', 'Materialize one planned repository when its files are needed.'],
  ...
  capability: {
    ...
    description: [
      ...
      '',
      'Repository onboarding starts from an exact credential-free Git URL. In VS Code, paste the URL',
      'first; Browse repositories is optional, fills that field, and runs the same read-only check.',
      '`capability onboard` returns one ref-bound preview with exact effects and preserved data;',
      'its plan ID must be confirmed',
      'before any write. Fresh setup creates sflow/config from installed defaults after one plan',
      'confirmation. Existing configuration changes require an exact diff and commit review.',
      'Recognized state can restore missing configuration from its',
      'verified mirror or retained history. Migrate reconciles packaged seeds only in valid current',
      'workflow-v2 configuration; it does not migrate workflow v1. Recreate builds the installed v2',
      'package while preserving verified portable organisation data. Offline reset-local removes',
      'only matching capability lead-registry and organisation-cache entries, not workspace/session',
      'registrations or FOS pins. All three modes require their own previews.',
      '',
      'A confirmed apply may return configuration-review-required, local-registration-pending, or',
      'ready-state-refresh-pending. Follow the returned review or retry action without repeating an',
      'already successful configuration publication. Selected snapshot clones are refused above',
      '16,384 local files or 128 MiB before checkout or parsing.',
      'Initial setup derived from mutable source refs publishes a review branch because Git cannot',
      'atomically guard those refs while creating sflow/config. `setup-proposals` lists these branches;',
      '`setup-proposal` inspects one; `setup-activate` checks its full commit and source receipt',
      'before an exact leased update. A direct attempt requires --acknowledge-unprotected; protected',
      'branches remain under the repository review process.',
      '',
      '`inspect-repository` remains a compatibility diagnostic that',
      'reports whether a repository is already mapped, known but unassigned, new, ambiguous, or',
      ...
      'When proposal inspection finds an exact unmodified Agent Markdown file from an older',
      'Singularity Flow package that conflicts with the current MCP policy, the returned',
      '`capability repair-proposal` action replaces only recognized historical package bytes on',
      'that proposal branch. It validates the joined workflow, agent, and MCP contract and creates',
      'a new proposal commit for review; it never activates the proposal or overwrites customized agents.',
      'An unrelated-history proposal is never merged or rebased. For a conflicting typed map',
      'proposal, `rebase-proposal` previews an exact source/target-bound record addition and',
      'requires the full source commit plus the returned plan ID before it creates a new review',
    ...
    options: [
      ...
      ['--dry-run', 'For onboard, inspect one repository and return a ref-bound plan without writing.'],
      ['--confirm-plan sha256:<PLAN>', 'For onboard, apply only the exact unchanged plan returned by dry-run.'],
    ...
    examples: [
      ['singularity-flow capability onboard <REPOSITORY-URL> --dry-run --json', 'Preview one safe setup, restore, or upgrade action without changing local or remote state.'],
      ['singularity-flow capability onboard <REPOSITORY-URL> --confirm-plan sha256:<PLAN> --json', 'Apply the exact unchanged repository-setup plan.'],
      ['singularity-flow capability setup-proposals --lead <URL> --json', 'List pending repository setup proposals, separate from capability map proposals.'],
      ['singularity-flow capability setup-proposal <SETUP-BRANCH> --lead <URL> --json', 'Inspect the exact setup commit, changed files, source freshness, and target.'],
      ['singularity-flow capability setup-activate <SETUP-BRANCH> --lead <URL> --confirm <FULL-COMMIT>', 'Confirm the reviewed setup commit; external review or an explicit direct-push acknowledgement is required.'],
      ...
      ['singularity-flow capability repair-proposal <REVIEW-BRANCH> --lead <URL> --confirm <FULL-COMMIT> --json', 'Safely refresh exact historical packaged agents on the proposal branch and return a new commit for review.'],
      ['singularity-flow capability rebase-proposal <REVIEW-BRANCH> --lead <URL> --json', 'Preview a safe record-level rebase without changing a ref.'],
      ['singularity-flow capability rebase-proposal <REVIEW-BRANCH> --lead <URL> --confirm <FULL-COMMIT> --confirm-plan <PLAN-ID> --json', 'Create a new review branch from the exact authority revision while retaining the source proposal.'],
