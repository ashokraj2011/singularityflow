...
4. Run `singularity-flow start <arguments>`.
5. Confirm that the current branch exactly equals the work ID.
6. Read the created `workflow.json`, `STATUS.md`, and `source.json`.
7. Summarize the source, current phase, required artifact, and next valid action.
8. Do not begin requirements work unless also requested. Recommend `/singularity-flow:requirements`.
