...
| Witnessed Engineering Loop | Exact-static local JUnit observation, immutable proposal binding, existing-approval review, migration, content-free benchmark, and isolated npm/VSIX engine proof are active; authenticated testcase claims and enforcement remain unavailable | Finish `WEL-P0-001` through `WEL-P0-003` external/cross-platform gates | [WEL pending work](WEL-PENDING-WORK.md) |
...
| Governed comprehension | Read-only, non-authoritative P0 code-local contract, closed registries, authority ADR, mutation tripwires, and isolated npm/VSIX proof exist | Finish the `CMP-P0-001` supported-platform corpus proof; keep record/enforce unavailable | [CMP roadmap](CMP-ROADMAP.md) |
