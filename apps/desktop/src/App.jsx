...
function Agents({ data, editor, setEditor, chooseAgent, saveEditor, createAgent, deleteFile, downloadFile, importAgent }) {
  ...
  const mappings = data.agentMappings;
  const explicitMappings = mappings.rows.filter((row) => row.source === 'configured').length;
  function chooseMappings() {
    setSpecialView('mappings');
