...
4. For a non-delegated publication, run the exact configured-producer publication command returned by the engine once. Never substitute producer or channel. Confirm sanitized `telemetry/<phase>-gen<N>.json`; use `--usage-json` only for exact usage.
